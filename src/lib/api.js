...
/** Cek status member otomatis berdasarkan riwayat transaksi nomor WA */
export async function cekStatusMember(nomorWa) {
  const { data, error } = await supabase
    .rpc('cek_status_member', { p_nomor_wa: nomorWa })
    .single()
  if (error) throw error
  return data
}
